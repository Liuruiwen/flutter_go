...
import 'bean/ArticleDataBean.dart';
import 'bean/ArticleListBean.dart';
 ...
 * Desc:文章列表
 */
class ArticleListWidget extends BaseFulWidget {
  String _title;
  String _id;

  ArticleListWidget(this._title, this._id);

  @override
  State<StatefulWidget> createState() {
    // TODO: implement createState
    return new ArticleListState(_title);
  }

  getList(bool isLoad, BuildContext context, int page, callBack(t)) async {
    print(ApiConfig.GO_HOME_LIST + page.toString() + "/json?cid=" + _id);
    var response = await HttpHelper().requestGet(
        context, isLoad,
        ApiConfig.GO_HOME_LIST + page.toString() + "/json?cid=" + _id);
    filtration(isLoad, context, response, (t) {
      callBack(ArticleListBean.fromJson(t));
    });
  }

}

class ArticleListState extends BaseWidget<ArticleListWidget> {
  ArticleListState(String titles) : super(titles);

  List<ArticleDataBean> _list = new List();
  int state = 1;
  int _mPage = 0;
  int pageCount = 0;
  ...
  Widget _getListItem(ArticleDataBean bean) {
